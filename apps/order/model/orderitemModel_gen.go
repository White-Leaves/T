...
var (
	...
	cacheGomallOrderitemIdPrefix = "cache:gomall:orderitem:id:"
)
...
func (m *defaultOrderitemModel) Delete(ctx context.Context, id int64) error {
	gomallOrderitemIdKey := fmt.Sprintf("%s%v", cacheGomallOrderitemIdPrefix, id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, gomallOrderitemIdKey)
	return err
}
...
func (m *defaultOrderitemModel) FindOne(ctx context.Context, id int64) (*Orderitem, error) {
	gomallOrderitemIdKey := fmt.Sprintf("%s%v", cacheGomallOrderitemIdPrefix, id)
	var resp Orderitem
	err := m.QueryRowCtx(ctx, &resp, gomallOrderitemIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultOrderitemModel) Insert(ctx context.Context, data *Orderitem) (sql.Result, error) {
	gomallOrderitemIdKey := fmt.Sprintf("%s%v", cacheGomallOrderitemIdPrefix, data.Id)
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("insert into %s (%s) values (?, ?, ?, ?, ?, ?, ?)", m.table, orderitemRowsExpectAutoSet)
		return conn.ExecCtx(ctx, query, data.Sn, data.UserId, data.ProductId, data.ProductName, data.CurrentPrice, data.Quantity, data.TotalPrice)
	}, gomallOrderitemIdKey)
	return ret, err
}
...
func (m *defaultOrderitemModel) Update(ctx context.Context, data *Orderitem) error {
	...
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.Sn, data.UserId, data.ProductId, data.ProductName, data.CurrentPrice, data.Quantity, data.TotalPrice, data.Id)
	}, gomallOrderitemIdKey)
	return err
}

func (m *defaultOrderitemModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheGomallOrderitemIdPrefix, primary)
}
